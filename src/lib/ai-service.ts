...
// Zero-click automated feedback: Extract data from invoice images
...
3. Materials used (list of items purchased)
...
${imagesText}`;
  ...
  try {
    const invoiceData = JSON.parse(response) as {
      totalCost: number;
      laborHours: number;
      materials: Array<{ name: string; quantity: number; unit: string; cost: number }>;
    };

    // Record learning feedback automatically (zero-click)
    await recordLearningFeedback(
      job.predictionId,
      job.id,
      { totalCost: invoiceData.totalCost || 0, laborHours: invoiceData.laborHours || 0 }
    );

    // Also record full job outcome
    await recordJobOutcome(
      jobId,
      {
        materials: invoiceData.materials.map(m => ({
          name: m.name,
          quantity: m.quantity,
          unit: m.unit,
          estimatedCost: m.cost
        })),
        laborHours: invoiceData.laborHours || 0,
    ...
    throw new Error("Failed to extract data from invoice images");
  }
}
